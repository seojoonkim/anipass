...
export default function Navbar() {
  ...
  return (
    <>
        ...
        <div className="max-w-[1180px] mx-auto px-3 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-10 md:h-12">
            ...
            <div className="flex items-center gap-0.5 sm:gap-1 md:gap-2 flex-shrink-0">
              ...
              {user && (
                <div className="relative" ref={notificationRef}>
                  <button
                    ...
                    className={`relative text-text-primary hover:bg-surface-hover p-1 min-[600px]:p-1.5 sm:p-2 rounded-md transition-colors flex items-center ${showNotificationDropdown ? 'bg-surface-hover' : ''}`}
                  >
                    <svg className="w-4 h-4 min-[600px]:w-4 min-[600px]:h-4 sm:w-5 sm:h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9" />
                    </svg>
                    {unreadCount > 0 && (
                      <span className="absolute -top-0.5 -right-0.5 bg-error text-white text-[9px] min-[600px]:text-[10px] font-bold rounded-full min-w-[14px] min-[600px]:min-w-[16px] h-[14px] min-[600px]:h-[16px] flex items-center justify-center px-0.5 animate-pulse">
              ...
              {/* Desktop User Menu (600px+) */}
              {user && (
                <div className="relative hidden min-[600px]:block" ref={userMenuRef}>
                  <button
                    onClick={() => setShowUserMenu(!showUserMenu)}
                    className="text-text-primary hover:text-text-secondary hover:bg-surface-hover text-xs sm:text-sm font-medium px-1 sm:px-1.5 md:px-2 py-1 sm:py-1.5 rounded-md transition-colors flex items-center gap-1 sm:gap-1.5 md:gap-2"
                  >
                    {user.avatar_url ? (
                      <img
                        src={getAvatarUrl(user.avatar_url)}
                        alt={user.display_name || user.username}
                        className="w-6 h-6 sm:w-7 sm:h-7 rounded-full object-cover border border-border"
                        onError={(e) => {
                    ...
                    <span className="text-xs sm:text-sm font-medium max-w-[60px] sm:max-w-[80px] md:max-w-[100px] truncate hidden min-[700px]:inline">{user.display_name || user.username}</span>
                    {(() => {
                      const levelInfo = getCurrentLevelInfo(otakuScore, language);
                      return (
                        <span
                          className="text-[9px] sm:text-[10px] px-1 sm:px-1.5 py-0.5 rounded-full font-semibold hidden lg:inline-flex"
              ...
              {/* Mobile User Menu - Visible below 600px */}
              {user && (
                <div className="relative min-[600px]:hidden" ref={mobileUserMenuRef}>
                  <button
                    onClick={() => setShowUserMenu(!showUserMenu)}
                    className="text-text-primary hover:bg-surface-hover p-1 rounded-md transition-colors flex items-center gap-1"
                  >
                    {user.avatar_url ? (
                      <img
                        src={getAvatarUrl(user.avatar_url)}
                    ...
                    <span className="text-[11px] font-medium max-w-[50px] truncate">{user.display_name || user.username}</span>
                    {(() => {
                      const levelInfo = getCurrentLevelInfo(otakuScore, language);
                      return (
                        <span
                        ...
                        >
                          <span style={{ color: levelInfo.color }} className="font-bold">{levelInfo.icon}</span>
                          <span style={{ color: levelInfo.color }}>{toRoman(levelInfo.rank)}</span>
                        </span>
                      );
                    })()}
      ...
      <div className="min-[600px]:hidden fixed bottom-0 left-0 right-0 bg-surface border-t border-border shadow-lg z-50">
      ...
      <div className="min-[600px]:hidden h-12" />
    </>
  );
}
